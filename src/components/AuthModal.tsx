  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        login(data.token, { user_id: data.user_id, nickname: data.nickname || "User", avatar: data.avatar }, data.expired_at);
        toast.success("Logged in successfully!");
        onClose();
      } else {
        const data = await authApi.register({ mobile, password, nickname });
        login(data.token, { user_id: data.user_id, nickname: nickname, avatar: data.avatar }, data.expired_at);
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {!isLogin && (
              <div className="flex flex-col space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-[#D7DADC]">Nickname</label>
            ...
            <div className="flex flex-col space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-[#D7DADC]">Mobile</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
            ...
            <div className="flex flex-col space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-[#D7DADC]">Password</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
